...
| `READY_FOR_REVIEW` | All three pipeline stages succeeded; an `EdarRecord` with `AI`-layer `EdarFieldValue` rows exists. Set by `csc_apps.processing.extraction_service` on successful extraction, as of Phase 6 (`docs/phase6-officer-review-approval.md` §10) — deferred through Phases 2-5 pending an actual review mechanism. |
| `IN_REVIEW` | Conceptually "an officer/reviewer has opened the record for review" (distinct from "ready" so two reviewers don't collide — see `docs/open-decisions.md` OD-001 for the exact review-assignment model). As implemented in Phase 6, this is a transient internal hop the approval action itself walks through on its way to `COMPLETED` — `GET /recordings/<id>/` stays strictly read-only and never triggers it, consistent with every other GET in this system. A dedicated "start review" action that sits in `IN_REVIEW` before approval is not implemented. |
